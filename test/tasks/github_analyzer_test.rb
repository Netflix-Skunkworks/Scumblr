...
class GithubAnalyzerTest < ActiveSupport::TestCase
  ...
  test "execute saved github search task with public members" do
    ...
    assert_equal(2, Result.last(2).first.metadata["vulnerabilities"].count)
    assert_equal(2, Result.last.metadata["vulnerabilities"].count)
    # check key_suffix is correct
    assert_equal("test", Result.last(2).first.metadata["vulnerabilities"].first["key_suffix"])
    # Check that the vulnerablity was opened
    assert_equal("Open", Result.last(2).first.metadata["vulnerabilities"].first["status"])
    ...
    # assert task id
    assert_equal(2, Result.last(2).first.metadata["vulnerability_count"]["task_id"]["58"])
    # assert key_suffix
    assert_equal(2, Result.last(2).first.metadata["vulnerability_count"]["key_suffix"]["test"])
  end
